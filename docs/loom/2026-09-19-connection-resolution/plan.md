...
intent: 2026-09-19-connection-resolution@4c5c3b7, amended @82d0722 and again during review
charter: 1.0
...
- Test: A1 positive: four-field-match-borrows; negative: port-mismatch-refuses. A2 positive: refusal-names-supplied-and-stored-targets; negative: exact-match-does-not-refuse. A3 positive: instructions-enumerate-borrowed; boundary: store-failure-falls-through-to-inline-refusal. A7 positive: every-new-test-red-against-base; boundary: base-source-collects.
...
- Files: src/redshift_comment_mcp/server.py, tests/test_server_resolution.py
- Test: A1 positive: blank-and-placeholder-port-still-borrow; negative: mistyped-port-refuses-to-borrow. A2 positive: refusal-names-each-profile-whole-target; boundary: two-profiles-differing-only-in-dbname-render-distinctly.
...
**W0-09 Finish W0-08: a truthful tie, an unforgeable name, a guard that covers the wire**  after: W0-08  acceptance: 3, 5, 8
- Files: src/redshift_comment_mcp/server.py, tests/test_server_resolution.py, tests/test_tools.py
- Test: A3 positive: single-match-still-borrows; negative: borrowed-mode-reports-no-phantom-profile. A5 positive: guard-covers-the-instructions-string; boundary: newline-in-profile-name-cannot-forge-a-line. A8 positive: tie-refusal-names-every-candidate; negative: identical-password-tie-still-refuses.
...
**W0-11 Close the code-lens round-1 findings**  after: W0-10  acceptance: 1, 5
- Files: src/redshift_comment_mcp/server.py, tests/test_server_resolution.py, tests/test_tools.py
- Test: A1 positive: ordinary-stored-port-still-matches; negative: unparseable-stored-port-refuses-to-lend. A5 positive: guard-scans-the-cli-too; boundary: exemption-only-inside-a-real-docstring.
- Risk: the launch side guarded a substituted port and the stored side did not, so the closed hazard re-entered through the other half of the comparison; agent-decided — mirror the guard rather than narrow the coercion.

**W0-12 Document the refusal the code gained after the docs task closed**  after: W0-11  acceptance: 6, 8
- Files: .claude-plugin/plugin.json, README.md, README.ja.md, README.zh-TW.md, src/redshift_comment_mcp/redshift_tools.py, tests/test_repo_invariants.py
...
2. Task splitting was agent-decided: ten accumulated items were cut to four tasks sharing one subject, W0-01 to W0-04. W0-05 to W0-12 were added during Build and review, each closing a defect in this change's own new surface — see 3c.
