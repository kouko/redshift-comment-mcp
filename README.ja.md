...
- **host / port / user / dbname を埋めて password だけ空欄 —— 既存
  ...
  埋めてください。一致するプロファイルが 2 つ以上あった場合も同様に
  接続を拒否し、該当する候補をすべて挙げます。パスワードのローテー
  ションで新旧のプロファイルが同じ接続先のまま残っているとよく起きる
  形なので、古い方を削除するかリネームしてください。
...
**パッケージ済みアーティファクトは 2 種類 — どちらか 1 つを選んでインストール：**

- **Claude Code プラグイン** — 6 個のスラッシュコマンドスキル **に加えて**
  MCP サーバーを提供し、`/redshift-setup` で設定します。上記のクイック
  スタートそのもので、Claude Code ユーザー向けの推奨パスです。
- **`.mcpb` Desktop Extension** — MCP サーバー **に加えて** Claude Desktop
  の GUI 設定フォームを提供しますが、スキルは **含みません**。
  `claude_desktop_config.json` を手で編集する代わりに、ワンクリック
  インストールと接続フォームが欲しい Claude Desktop ユーザー向けのパスです。
...
**`.mcpb` の前提条件 — `uv`。** Desktop Extension は `uv` ランタイム上で
動くため、`.mcpb` をインストールする前にマシンに `uv` が必要です
（macOS なら例えば `brew install uv`）。Claude Code プラグインのパスでは
この手順は不要です — Claude Code が自前の `uv` を提供します。
...
| `~/.config/redshift-comment-mcp/config.toml.lock` | 空のロックファイル。POSIX のみ（`fcntl` が使えない Windows では作られず、書き込みは直列化されない）。プロファイルの書き込み全体と、削除のうち config.toml を書き換える部分で保持され、同時に走った 2 つの操作が互いのプロファイルを消し合わないようにする。キーチェーンの操作は OS のロック解除ダイアログで人間の時間だけ止まりうるので、ロックの外で走る。最初の書き込みで作成されそのまま残る。消してしまっても害はない。書き込みは config ディレクトリ自体にもロックを取っており、このファイルを削除してもそちらは切り離せないので、その瞬間ロックを保持していた書き込みは保護されたままで、ファイルは次の書き込みで作り直される。 | `0600` |
