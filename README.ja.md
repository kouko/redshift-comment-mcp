...
| `~/.config/redshift-comment-mcp/config.toml.lock` | 空のロックファイル。POSIX のみ（`fcntl` が使えない Windows では作られず、書き込みは直列化されない）。プロファイルの書き込み全体と、削除のうち config.toml を書き換える部分で保持され、同時に走った 2 つの操作が互いのプロファイルを消し合わないようにする。キーチェーンの操作は OS のロック解除ダイアログで人間の時間だけ止まりうるので、ロックの外で走る。最初の書き込みで作成されそのまま残る。基本はそのまま置いておくこと。書き込み中のサーバーがそのダイアログの後ろで待っていることがあり、「何も動いていない」かどうかは外からは判断できない。消しても次の書き込みで作り直されるが、その瞬間ロックを保持していた書き込みは保護を失う。 | `0600` |
