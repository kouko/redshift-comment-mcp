...
と、その上に 5 個のスラッシュコマンドスキルを乗せた Claude Code プラグ
...
### スラッシュコマンドスキル（5 個、[`skills/`](skills/) で定義）
...
├── skills/                                         5 つのスラッシュコマンドスキル — 専用 README あり
...
- [`skills/README.md`](skills/README.md) — 5 スキルの全体像
...
- `/redshift-cache-schema` がテーブル単位の `.md` ファイルを書き出す
  ため、消費側スキルは Read ツールで直接読む（MCP のレスポンス経路を
  完全にバイパス）。一度プライムすれば以降のメタデータ参照はサイズ
  制限の外側で動作する。
